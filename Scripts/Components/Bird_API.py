...
# reverse engineering access to Bird Scooter API--
# need to 'create' new email address to get a new token
# every time we run the script
login = {"email": "ernieels69@gmail.com"}
headers = {"Device-id": "123E4567-E89B-12D3-A456-426655440060", 
           "Platform": "ios", "Content-Type": "application/json"}
response = requests.post('https://api.bird.co/user/login', 
...
for city in cities:
    ...
    results = requests.get(
            "https://api.bird.co/bird/nearby?latitude="
            +str(lat)+"&longitude="+str(lon)+"&radius=10000", 
            headers = headers2)
    ...
    df2 = pd.concat([df2.drop(['location'], axis=1),
                    df2['location'].apply(pd.Series)], axis=1)
